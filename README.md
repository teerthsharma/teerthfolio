...
Kept here on purpose; each was caught by a capture or by the owner, not by a
test, until a rule was added.

- **A horizon that walled the island.** A ring of peaks riding the camera 31 m
  out was meant to sit on the horizon, but the follow camera's top edge is 24.5°
  below it, so the ring stood inside the island as flat blue walls round the
  seal. Deleted.
- **Clouds, twice.** Puffs riding the camera covered the top half of every frame
  and hid the northern landforms; a rebuild parked at the camera's focus was
  never visible at all. There is nowhere in this camera's view for a cloud that
  does not cover land, so there are none.
- **Explaining.** The first landforms and buildings retold the landing site's
  figures as 3D diagrams, cube grids and headline text. The owner: "this
  explains mujoco, I don't want that, that's GitHub pages". All of it came off.
- **A plush Rushmore.** The first carved faces read as beige plush busts in
  front of a cliff ("looks like squirrel", "chopped"): a neck almost as wide as
  the head, pink cheek blobs, ears. Rebuilt as relief carved out of one granite
  massif.
- **Half-size seals.** The colony built every part from a sphere of radius 0.5
  scaled by semi-axes, so bodies came out half size, heads hung off them and the
  eyes floated; the flippers drew black for want of vertex colour. Rebuilt from
  the player pup's own geometry.
- **A ping-pong seal.** At 15 m/s with a 5.8 m glide and 0.35 restitution the
  seal crossed the island in 11 s and bounced off walls, so nobody saw the
  buildings play. Now 10 m/s, a 2.9 m glide, 0.12 restitution, half speed near
  any place.
- **A river that looked slow.** The riding seal moved at up to 28 m/s while the
  drawn streaks moved at 0.8x the current, so the rider overtook the water 2.5
  times over. The streaks now travel at 2.3x.

## 9. Limits

Frame time has not been re-measured on the development machine (an Intel UHD
integrated GPU) since the carved mountain (about 134k triangles), the metal dam,
the 18 costumed pups and the hero-move effects were added; the last reading
there was p50 30.3 ms. The radiation screen effect lives in the post chain, so a
device the performance monitor drops to its lowest tier sees the mutation
without the flood. Every capture named in the commit history was taken in
headless Chrome with the GPU enabled, not on a phone.

...
vec3 polarSkyAnchor(vec3 direction) {
  ...
  vec3 dawnKiss = vec3(0.910, 0.608, 0.373) * horizonBand * (0.3 + 0.7 * pow(sunAmount, 3.0)) * lightShift;
  float sunDisc = smoothstep(0.9992, 0.99975, sunAmount);
  float sunHalo = pow(sunAmount, 22.0);
  anchor += dawnKiss * 0.55 + vec3(1.0, 0.85, 0.60) * (sunDisc * 0.85 + sunHalo * 0.2 * lightShift);
  ...
  // as absent). Broad folds carry the shape, fine streaks give vertical striation.
  float curtainFold = biomeFbm3(vec2(auroraDelta * 8.0 + uTime * 0.020, direction.y * 2.2 + uTime * 0.012)) * 0.5 + 0.5;
  float curtainStreak = biomeFbm2(vec2(auroraDelta * 24.0 - uTime * 0.030, direction.y * 1.3)) * 0.5 + 0.5;
  float curtain = smoothstep(0.36, 0.72, curtainFold * 0.58 + curtainStreak * 0.42);
  vec3 auroraTint = mix(
    vec3(0.4353, 0.9059, 0.7843),
    vec3(0.5529, 0.4118, 0.8392),
    smoothstep(0.06, 0.48, direction.y)
  );
  anchor += auroraTint * (auroraSector * auroraFoot * auroraRise * curtain * 0.55);
  return anchor;
}
...
void main() {
  ...
  if (vBiomeRole > 0.5 && vBiomeRole < 1.5) {
    ...
    vec3 primaryTopologyColor = mix(groundPrimarySecondary, groundPrimaryAccent, 0.82);
    vec3 secondaryTopologyColor = mix(groundSecondarySecondary, groundSecondaryAccent, 0.82);
    color = mix(
      color,

import {
  ...
  POLAR_LIGHT_POOL_GLSL,
  POLAR_PROP_CONTACT_COUNT,
} from "./polar-ground.js";
/**
...
const profiles = {
  ...
  "s2-kernel-core": {
    ...stationPersonalityBiomeIdentity("s2-kernel-core", {
      ...
      light: { azimuth: 115, elevation: 29, fillStrength: 0.28, rimStrength: 0.58 },
...
export const POLAR_BIOME_QUALITY = deepFreeze({
  ...
  // Medium carries high's content. The world is fill-bound, so geometry and
  // shader detail are close to free while resolution is not: raising this tier
  // from 64/24/0.72 to high's 96/32/1.0 left the settled frame at 14.5-14.6ms
  // across three runs against 14.5-14.9ms before, and measured +2.9% more
  // high-frequency detail against a frozen-clock control, with low unchanged in
  // the same run as a null. Medium and high now differ in resolution alone.
  medium: {
    terrainSegments: 96,
    skySegments: [48, 24],
...
// Stage split, attempted and reverted — the analysis is worth keeping.
//
// Of this block's 9,245 characters across 20 functions, only 2,288 are reachable
// from the fragment shader. evaluateBiomeField and the eight authored station
// fields it dispatches to — qpuLead, aetherRibbon, topologyStrata, magneticSalt,
// assemblyRunway, s2Pressure, plaque, upstreamSignal — plus their private
// helpers total 6,725 characters that only the vertex stage calls, because they
// feed vPrimaryField and vSecondaryField which the vertex stage interpolates.
// Both programs nonetheless carry all of it, so ANGLE translates ~6.7KB of
// unreachable code twice on every cold visit.
//
// Splitting it into a shared half and a vertex-only half took the fragment from
// 61,730 to 54,779 characters and total main-thread blocking from 9,046ms to
// 3,055ms, with the biome programs dropping out of the blame list entirely. It
// also broke the terrain: the ground rendered as a flat dark plane with no snow,
// no relief and no atmosphere. gl.debug.checkShaderErrors is off — deliberately,
// it is what made the cold boot 16.5s — so a damaged shader fails silently and
// the measurement looked like a 3x win.
//
// No kept function calls a moved one, so the dependency graph was not the
// problem; the mechanical extraction damaged the string. Whoever does this
// properly should extract by explicit function list into a new constant rather
// than by cutting text out of this one, and should verify the render before
// trusting any timing.
// Link cost is not concentrated in any one construct here, which is worth knowing
// before anyone optimises this file for it. polar-biome-world-solid links in
// 2,570ms on a cold visit and that link is what freezes the world for ~3.9s just
// after it appears. Deleting the most expensive-looking thing in the shader — the
// 3x3 nested Worley loop below, which ANGLE unrolls into nine inlined blocks —
// took the fragment from 4 loops to 2 and 619 characters lighter, and bought:
//
//   solid  2,570ms -> 2,405ms      sky  1,608ms -> 1,505ms
//
// Six percent. The cost is spread across a 56KB program rather than sitting in a
// hot spot, so there is no micro-optimisation here worth breaking the terrain
// for. Anything that moves this number meaningfully has to make the program
// substantially smaller — specialising it per tier the way BIOME_ROLE_SKY
// specialises it per role is the shape that would work — and that is an
// art-direction change, not a shader tidy-up.
const BIOME_FIELD_SHARED_GLSL = String.raw`
...
const BIOME_FIELD_VERTEX_GLSL = String.raw`float biomeFbm4(vec2 point) {
  float value = 0.0;
  float amplitude = 1.0;
  mat2 octaveRotation = biomeOctaveRotation();
...
const BIOME_FIELD_GLSL = `${BIOME_FIELD_SHARED_GLSL}
${BIOME_FIELD_VERTEX_GLSL}`;
...
uniform float uGeographyAbsent;
...
void main() {
  ...
  if (aBiomeRole < 0.5) {
    ...
    // The sheet arrives as PlaneGeometry, whose normal is a constant up vector,
    // so until here every lighting term on the largest surface in frame
    // evaluated to the same number everywhere and the displaced form was
    // invisible. The shared ground field knows its own slope; ask it.
    localNormal = polarGroundNormal(worldPosition.xz);
...
// Contact skirts for the docked station's local geography props: xy world XZ,
// z squared skirt radius, and the field's own bounding circle that gates the
// whole loop. See POLAR_PROP_CONTACT_COUNT in lib/polar-ground.js.
uniform vec3 uPropField;
uniform vec3 uPropContacts[${POLAR_PROP_CONTACT_COUNT}];
...
${BIOME_FIELD_SHARED_GLSL}
// Declared after the shared field block: the traveler caster rides
// polarGroundHeight, and GLSL has no forward declarations.
${POLAR_HERO_SHADOW_GLSL}
${POLAR_LIGHT_POOL_GLSL}
...
vec3 polarSkyAnchor(vec3 direction) {
  ...
  // The dawn geometry is hoisted above the gradient because the sky's HUE now
  // depends on it, not just its brightness: which way the sun lies decides
  // whether a bearing is honey or ice. Full sun side within ~23 degrees of the
  // bearing, gone by ~87. Measured against the solved camera composition at
  // 1440x900, only two of the eight station framings point inside that cone --
  // observatory-plaque is 4 degrees off the sun and s2-kernel-core 22 -- while
  // the other six sit 49 to 159 degrees away. So the warm end is something you
  // turn toward, and most of the world gets the ice end.
  vec3 sunDir = normalize(vec3(0.904, 0.235, -0.426));
  float sunAmount = max(dot(direction, sunDir), 0.0);
  float sunSide = smoothstep(0.05, 0.92, sunAmount);
  float horizonBand = exp(-pow(max(direction.y, 0.0) / 0.15, 2.0));
  float bloomBand = exp(-pow(max(direction.y, 0.0) / 0.26, 2.0));

  // GRADIENT RANGE. This is why the sky read as grey-lilac haze.
  //
  // The playable camera sits low, so the sky this world actually frames is a
  // shallow strip: direction.y runs 0 to 0.070 at observatory-plaque (4.0
  // degrees of sky) and 0 to 0.276 at qpu-ice-bridge (16.0 degrees), the widest
  // of the eight. The ramp here used to be smoothstep(-0.12, 0.72), so between
  // 13% and 46% of its range ever reached the screen and the authored zenith
  // colour was never visible at any station. What the frame actually showed was
  // the horizon stop under the haze shelf resolving toward one pale constant --
  // and that constant was the grey-lilac.
  //
  // So the stops are compressed into the strip that is on screen. The deepest
  // one is still half absent at 0.24 and only completes at 0.30, which keeps it
  // off every station but qpu-ice-bridge and confines it there to the top rows;
  // a trial that let it land lower cost that station 29.7 sky luma.
  //
  // The three stops carry the SAME chroma (0.29) and differ in hue and value:
  // 194 -> 203 -> 212 degrees while luma falls 218 -> 198 -> 180. That split is
  // measured, not stylistic. The post pass runs a midtone vibrance band that is
  // bit-identical at or below 0.30 saturation and lifts hard above it, so sky
  // chroma authored here arrives on screen roughly doubled: a first cut that
  // authored 0.49 saturation at 16 degrees measured 0.75 in frame and cost that
  // band 18.6 luma, which is a colour wash, not a sky. Hue and value are the
  // axes with headroom; chroma is not, and the grade owns it.
  float altitude = smoothstep(0.015, 0.30, direction.y);
  vec3 seamCyan = vec3(0.640, 0.845, 0.910);
  vec3 midTeal = vec3(0.609, 0.766, 0.863);
  vec3 tealIndigo = vec3(0.589, 0.703, 0.833);
  vec3 anchor = mix(seamCyan, midTeal, smoothstep(0.0, 0.50, altitude));
  anchor = mix(anchor, tealIndigo, smoothstep(0.80, 1.0, altitude));
  ...
  //
  // The shelf used to resolve to the single constant vec3(0.760, 0.795, 0.885)
  // named above. It now splits by bearing, and both ends are BRIGHTER and less
  // saturated than the grey they replace -- which is the only way the seam can
  // gain hue at all, because the snow/sky anchor pool the colour contract
  // measures (saturation <= 0.28, luma >= 100, >= 35% of frame) lives almost
  // entirely in the bottom 0-3 degrees of sky and the ground just under it.
  float hazeShelf = exp(-max(direction.y, 0.0) / 0.085);
  vec3 seamWarm = vec3(0.985, 0.905, 0.775);
  vec3 seamCool = vec3(0.780, 0.885, 0.955);
  anchor = mix(anchor, mix(seamCool, seamWarm, sunSide), hazeShelf * 0.60);
  // Hue, not gain. Amplifying the additive dawn lobes below was measured and
  // does not work: at the sun bearing the sky is already clipped (saturation
  // 0.049, luma 242) with boundPolarHighlights compressing everything above it,
  // so a bigger gain buys a bigger white blob and no colour. The gains are
  // therefore CUT -- 0.40 to 0.26 core, 0.20 to 0.13 bloom -- and the headroom
  // is spent on these two mixes, which move chroma at fixed exposure. Both are
  // gated on sunSide, so the six stations that never frame the sun pay nothing
  // for them and keep a clean ice horizon.
  anchor = mix(anchor, vec3(0.992, 0.862, 0.690), horizonBand * sunSide * 0.36);
  anchor = mix(anchor, vec3(0.930, 0.640, 0.620), bloomBand * sunSide * 0.18);
  ...
  // The bearing, its sun-side falloff and both gaussian bands are computed at
  // the top of this function, because the gradient itself now reads them.
  ...
  anchor += dawnCore * 0.26 + dawnBloom * 0.13 + vec3(1.0, 0.85, 0.60) * (sunDisc * 0.85 + sunHalo * 0.2 * lightShift);
  // GEOGRAPHY STAND-IN. The low tier runs geographyInstances 0, so the distant
  // massifs -- the brightest large object in frame -- are simply not drawn. Measured
  // on one station across the ladder at a fixed camera: low 102.9 mean luma against
  // medium 123.7 and high 126.4, with the >150 bright fraction falling 16.8% to 11.4%.
  // That 21-luma hole put low under this project's own 108 floor and then cascaded,
  // because a darker frame has proportionally fewer high-luma low-saturation pixels
  // and so fails the snow/sky anchor ratio and inflates the accent fraction with it.
  //
  // The fix is NOT to draw the geography at low: it is off because those machines
  // cannot afford the draw or the fill, and that judgement stands. What the massifs
  // contributed was LIGHT in a band hugging the horizon, so the band carries it
  // instead -- no geometry, no draw call, one multiply-add on the sky program only.
  //
  // Near-neutral and very slightly cool, because that is what the massifs read as at
  // distance under aerial haze; a warm lift here would reproduce the dawn core and
  // push the horizon orange. Confined to horizonBand so the zenith is untouched and
  // the sky keeps its top-to-bottom gradient.
  // Sized by measurement against the DARKEST station, not the first one tested.
  // Tuning history, one station per line, mean canvas luma at a fixed camera:
  //   0.150  topology-archive-wall 102.9 -> 107.3   (still under the 108 floor)
  //   0.300  topology-archive-wall      -> 109.7    but assembly-tool-locker 104.4, still under
  //   0.480  sized so the darkest station clears with headroom rather than sitting on the line
  // Medium and high measured byte-identical across all three settings (123.7 / 126.4),
  // which is the check that matters: the term is confined to the tier that drops the
  // geography and cannot leak into the tiers that draw it.
  //
  // It deliberately does NOT close the whole ~21-luma gap to medium. Low is allowed to
  // be a different picture — fewer draws, no distant massifs, no dynamic weather — it
  // is just not allowed to be a failing one.
  anchor += vec3(0.480, 0.506, 0.550) * horizonBand * uGeographyAbsent;
...
void main() {
...
#else
  {
    ...
    if (vBiomeRole > 1.5) {
      ...
      // NOTE: this whole branch is vBiomeRole > 1.5, the local geography props, which
      // the low tier does not draw at all (geographyInstances 0). Scaling an additive
      // light term by uShaderDetail looks like the tier-brightness bug the sastrugi
      // carve below documents, and it was changed on that basis and measured: mean luma
      // at low moved 0.0. It is unreachable at the only tier where it would matter, so
      // it is left exactly as authored. The low-tier ground deficit is somewhere else.
...
#ifdef BIOME_ROLE_SKY
    color = mix(color, atmosphereTint, weather * 0.22);
#else
...
#ifndef BIOME_ROLE_SKY
  {
    ...
    vec3 shadedNormal = normalize(vWorldNormal);
    float wrappedLight = clamp(dot(shadedNormal, blendedLightDirection) * 0.5 + 0.5, 0.0, 1.0);
    float toonLight = floor(wrappedLight * 4.0 + 0.5) / 4.0;
    // The terrain now carries a real surface normal instead of PlaneGeometry's
    // constant up, and a 4-step ramp across a 58-unit sheet quantises a smooth
    // dune into hard terraces -- posterisation that reads as a banding artifact
    // rather than as form. The props keep the toon ramp their faceted
    // silhouettes are drawn for; the ground takes the same light continuously.
    float isTerrain = step(vBiomeRole, 0.5);
    color *= 0.84 + mix(toonLight, wrappedLight, isTerrain) * 0.18;
    if (vBiomeRole < 0.5) {
      // SUN-RAKED RELIEF. This is the term the ground was missing, and it is
      // light rather than texture on purpose: a polar field reads through how a
      // low sun grazes large-scale form, and the house rule is that floor detail
      // stays subordinate to the hero. Nothing here is high-frequency; it is the
      // dune field and the drift banks finally catching the key.
      //
      // Driven by the DEVIATION of the surface normal from flat, not by the
      // absolute dot product. blendedLightDirection.y is exactly what a flat
      // patch returns, so subtracting it leaves a signed slope response whose
      // mean over the field is the gradient's mean, which for a sum of sines is
      // zero. That is the same law the sastrugi carve below is written to --
      // a new term may control variation, never the mean -- and here it is also
      // what keeps the snow anchor ratio where the colour-continuity contract
      // needs it: the field gets lighter and darker in equal measure.
      // The gain is large because the quantity is small: a snow field's slopes
      // are a few degrees, and the whole dune field spans a normal deviation of
      // about 0.23. At unity this term moves the ground by under one 8-bit step
      // and the surface stays the flat sheet it was.
      //
      // Saturated smoothly rather than clamped, and that is not a detail. At a
      // gain this size a clamp is reached across a large fraction of the field,
      // which replaces relief with two flat plateaus at full darkening and full
      // lift -- posterisation wearing a lighting term's name, and measurably
      // expensive: the plateau cost the bottom band of the frame 25 mean luma
      // and 639 of its 677 snow-anchor pixels at the darkest station. x/(1+|x|)
      // is odd, so it preserves the zero mean the luma balance below depends on,
      // it is two ALU with no transcendental, and it never fully pins -- the
      // steepest flank in the field reaches 0.62, not 1.0.
      float rakeResponse =
        (dot(shadedNormal, blendedLightDirection) - blendedLightDirection.y) * 7.0;
      float rake = rakeResponse / (1.0 + abs(rakeResponse));
      // 0.22, not the 0.26 the relief alone would want. At 0.26 the darkest
      // station landed on exactly 35.0% against the colour contract's 35% snow
      // anchor floor, and a gate passing with no margin is a gate that fails on
      // the next unrelated palette change. This is the one number in this block
      // that is a budget rather than a judgement.
      color *= 1.0 + rake * 0.22;
      // Snow reads by VALUE first and by the one hue step polar snow actually
      // has: a lee slope out of the sun falls to blue, a sun-facing slope does
      // not turn warm so much as it turns to bare white crust.
      //
      // These two are HUE ONLY. Both targets sit at luma 0.805, matched to each
      // other and to the snow base they tint, so the multiply above is the only
      // term here that moves value.
      //
      // That separation is what makes this affordable. Written as a darker blue
      // and a brighter white they each pulled value in the same direction the
      // multiply already had, so a lee pixel was darkened twice and a sunward
      // one lifted twice. The pair still balanced on average, but the anchor
      // ratio the colour contract measures is a THRESHOLD, not a mean: it counts
      // pixels above luma 100, and the darkest station's frame averages 100, so
      // doubling the excursion either side of that line costs far more pixels
      // below it than it wins above. Value in one term, hue in another, and the
      // saturations stay at 0.15 and 0.13 -- well under the 0.28 that contract
      // uses to tell snow from colour, which is also just what polar snow is.
      color = mix(color, vec3(0.755, 0.812, 0.888), max(-rake, 0.0) * 0.26);
      color = mix(color, vec3(0.852, 0.802, 0.740), max(rake, 0.0) * 0.26);
      ...
      // Normalised by the field's own peak rather than by two constants that
      // encoded the amplitude this field happened to have when the term was
      // written. 0.22 is the terrain mesh's Y offset in PolarBiomeWorld.jsx,
      // which is what makes vWorldPosition.y a surface height again.
      ...
      // SPINDRIFT CREEP, and it is one vec2 add rather than a new term because
      // the sheet already had everything except a clock.
      //
      // Two frames a second apart at the home dome measured 88% of the picture
      // bit-frozen, and what moved was the aurora band along the top edge -- so
      // the whole lower two thirds, which is the largest thing in frame, was a
      // still photograph. The obvious fixes all add: a particle layer, a second
      // noise octave, a scrolling overlay. This adds nothing. driftBand and
      // scourLine below are already sampled from this one warp, so translating
      // its SAMPLE POINT slides both scales of the carve across the sheet
      // together, which is what a katabatic actually does to a snow surface.
      //
      // It is a translation of a stationary field, so it moves the carve without
      // moving its mean -- the same law the uShaderDetail mix below is written
      // to, and the reason this cannot cost the colour contract its snow anchors.
      //
      // THE RATE IS CALIBRATED, NOT CHOSEN. It was first authored at
      // (0.031, -0.018), which is 0.094 world units per second -- about two
      // minutes for a drift band to cross its own 10-unit wavelength. Measured
      // against the standard's own instrument (two frames ONE SECOND apart in a
      // single session, three pairs, median) that is invisible: the lower two
      // thirds moved 19.83 / 31.03 / 15.22 percent of pixels by more than 4/255
      // against a same-build repeat spread of 0.63 / 0.80 / 2.38 points, so two
      // of the three stations were inside their own noise. A drift slow enough
      // that a one-second window cannot see it does not meet a standard written
      // about one-second windows, whatever it looks like in a still.
      //
      // 5x that, 0.47 and 0.27 world units per second, is the band that shows in
      // a second and still takes ~50 seconds to cross the visible field. Faster
      // reads as crawling texture, which the house rule on floors rules out
      // ahead of any of this.
      //
      // uTime is pinned to 0 by the caller under reduced motion (see shaderTime
      // in PolarBiomeWorld.jsx), so this freezes to exactly the authored scatter
      // rather than needing a gate of its own.
      float scourWarp = biomeFbm2(vWorldXZ * 0.33 + vec2(11.3, 4.7) + uTime * vec2(0.155, -0.090));
      ...
      // A directional version of this carve was tried here and removed. sin() is
      // symmetric about each ridge, so reading these two scales through it
      // darkens the windward and lee walls equally; cos() is the same field's
      // slope along the wind bearing both scales share, and weighting it by the
      // key gave the lit/lee split the sun-raked term above cannot produce on a
      // graded shelf, where the height field is masked to zero by design.
      // Measured against a same-code repeat run it moved edge energy in none of
      // nine ground regions at any of four octaves -- against a repeat noise
      // floor under 1.5% -- for two transcendentals per fragment on the largest
      // surface in a fill-bound world. The near-field form that the shelf was
      // missing is carried by the drift bank instead, which is geometry the
      // ground field already had and which the surface normal now lights.
      float sastrugiCarve = driftBand * 0.55 + scourLine * 0.45;
      // Detail controls VARIATION, not mean. Scaling the whole darkening term by
      // uShaderDetail also scaled its average, so the low tier — which runs 0.34
      // against high's 1.0 — painted snow 33 luma brighter before the grade even
      // ran (214 against 181, measured with qa-no-post at the same spot). That
      // is the tier difference the mascot has to be legible against, and no
      // amount of exposure or light-rig work can reach it. Mixing toward the
      // field's own mean of 0.5 keeps the average carve identical at every tier
      // and lets detail do only what it is named for.
      color *= 1.0 -
        (0.055 + 0.085 * crestWeight) * mix(0.5, 1.0 - sastrugiCarve, uShaderDetail);
      color += vec3(0.032) * mix(0.5, sastrugiCarve, uShaderDetail) * crestWeight;
      ...
      // The wind turns the facets too. The grain field above is deterministic
      // per cell and the fire test is a bearing test, so advancing each grain's
      // facet at its OWN rate makes glints wink in and out where the sheet is
      // already sparkling -- no new samples, no new hash, one multiply-add on an
      // angle that was already being computed. Per-grain rate rather than one
      // shared clock, or 9% of the sheet flashes in unison and reads as a strobe
      // instead of as snow. Freezes with uTime under reduced motion.
      float facetAngle = sparkleSeed * 251.33 + uTime * (0.30 + 0.55 * sparkleSeed);
    ...
    // Spill, applied after the shadow so snow in a building's own shadow still
    // catches the light coming out of it. Additive and small: this is a light
    // source reading on the ground, not a second route for station identity to
    // dye the field, which neutralizeGroundColor exists to prevent.
    color += polarGroundLightPools(vWorldXZ) * 0.24 * (1.0 - horizonFade);
  }
#endif

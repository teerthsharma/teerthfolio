...
export function createSeal(x, z, heading = Math.PI) {
  return { x, z, vx: 0, vz: 0, heading, speed: 0, impact: 0, throttle: 0, skid: 0, water: 0, air: 0, flight: 0, whirled: 0 };
}
...
export function stepSeal(seal, { input, target, boost }, dt, world) {
  // THE WHIRLPOOL (world.whirlpool, lib/world/river.js WHIRLPOOL). Thrown, the
  // seal flies ballistic with a spin, `air` rising and falling 0..1 for the
  // renderer, and lands with a thump that takes most of its speed. In the
  // water under the funnel it is caught and carried round the pin, tighter
  // and faster, then thrown back onto the island.
  const whirl = world.whirlpool;
  if (whirl && seal.flight > 0) {
    seal.flight = Math.max(0, seal.flight - dt);
    seal.air = Math.sin(Math.PI * (1 - seal.flight / whirl.flight));
    seal.x += seal.vx * dt;
    seal.z += seal.vz * dt;

...
export const MOTION = {
  maxSpeed: 11, // the island is 140 m across: about 13 s at a slide, 8.5 s dashing
  boost: 1.5, // 16.5 m/s boosted
  accel: 3.0, // 1/s toward the wanted velocity: 90% of top speed in 0.77 s
  brake: 1.9, // 1/s with no input: glides 5.8 m from top speed, 90% stopped in 1.2 s. This is the ice.
  ...
  arrive: 7.8, // m, click-to-move slows inside this distance (scaled with maxSpeed: same damping, no overshoot)
  sealRadius: 0.9,
  sealMass: 4,
  propFriction: 1.4,
  restitution: 0.35,
...
export function stepSeal(seal, { input, target, boost }, dt, world) {
  ...
  const river = riverAt(seal.x, seal.z);
  ...
  if (water > 0) {
    ...
    if (!throttle && flow > 1e-3) {
      const px = -flowZ / flow;
      const pz = flowX / flow;
      const side = riverAt(seal.x + px * 0.5, seal.z + pz * 0.5).depth > water ? 1 : -1;
      const off = (1 - water) * (RIVER.width / 2); // m from the centre line
      flowX += px * side * off * MOTION.centring;
      flowZ += pz * side * off * MOTION.centring;
    }
  }
  seal.water = water;

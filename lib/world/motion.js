...
export const MOTION = {
  maxSpeed: 15, // the island is 168 m across: about 11.4 s at a slide, 7.2 s dashing
  boost: 1.6, // 24 m/s boosted
  accel: 3.4, // 1/s toward the wanted velocity: 90% of top speed in 0.68 s
  brake: 2.6, // 1/s with no input: glides 5.8 m from top speed, 90% stopped in 0.89 s. This is the ice.
  ...
  arrive: 10.6, // m, click-to-move slows inside this distance (scaled with maxSpeed: same damping, no overshoot)
  sealRadius: 0.9,
  sealMass: 4,
  propFriction: 1.4,
  restitution: 0.35,
...
export function stepSeal(seal, { input, target, boost }, dt, world) {
  ...
  // A seal that walks onto a bridge deck stays dry on it; a swimmer passes under.
  const water = river.inside && !(seal.water === 0 && onBridge(seal.x, seal.z, river)) ? river.depth : 0;
  let flowX = 0;
  let flowZ = 0;
  if (water > 0) {

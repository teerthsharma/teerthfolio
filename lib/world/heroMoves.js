...
//   domain  a Gojo-style levitation inside an expanding void
...
const BY_ID = {
  "pr-nemo-relay-481": "domain", // the NVIDIA moat
  "pr-topograph-432": "domain",
  "pr-triton-kernels-22": "domain",
  "pr-mujoco-3396": "domain",
...
export function heroMoveFor(place) {
  ...
  return "domain"; // blues and purples
}
...
export function heroPose(move, u, place, sx, sz, out) {
  ...
  if (move === "orbit") {
    ...
    const k = smooth(0.2, 0.4, u) * (1 - smooth(0.8, 0.95, u));
    out.y = 2.2 * k;
    out.yaw = toPlace + Math.PI * 4 * smooth(0.2, 0.85, u);
    out.k = k;
  } else if (move === "beam") {
    out.yaw = toPlace;
    out.k = u > 0.2 && u < 0.9 ? 1 : 0;
    const recoil = 0.5 * smooth(0.45, 0.5, u) * (1 - smooth(0.5, 0.8, u));
    out.x = sx - Math.sin(toPlace) * recoil;
    out.z = sz - Math.cos(toPlace) * recoil;
  }
  return out;
}

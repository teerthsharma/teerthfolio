 ...
 *
 * Sized against the block, not eyeballed. A course block is 0.205 deep, so the
 * old 0.15 let one block stand 0.15 proud of a neighbour seated at 0.009 —
 * three quarters of its own depth. That is not a hand-laid wall, it is rubble
 * stuck to a ball, and it is the single largest reason a still of this dome
 * read as loose plates: at the low tier's 5 courses of very large blocks each
 * one was visibly at its own random depth, and the sky showed through the steps
 * between them. 0.038 is ~18% of block depth, which is the tolerance a real
 * snow block is cut and set to: the outline stays bumpy rather than
 * mathematically smooth, and the wall reads as one surface again.
 */
export const POLAR_DOME_BLOCK_SEAT_JITTER = 0.038;

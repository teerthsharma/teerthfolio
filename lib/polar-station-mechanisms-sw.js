...
export const ASSEMBLY_LOCAL_OPEN_FACE_XZ = Object.freeze([-1, 0]);

export function deriveAssemblyVisitorYaw(
  centerXZ,
  dockXZ,
  localOpenFaceXZ = ASSEMBLY_LOCAL_OPEN_FACE_XZ,
) {
  const dockX = finite(dockXZ?.[0]) - finite(centerXZ?.[0]);
  const dockZ = finite(dockXZ?.[1]) - finite(centerXZ?.[1]);
  const localX = finite(localOpenFaceXZ?.[0], -1);
  const localZ = finite(localOpenFaceXZ?.[1]);
  if (Math.hypot(dockX, dockZ) <= STEP_EPSILON) return 0;
  return Math.atan2(dockX, dockZ) - Math.atan2(localX, localZ);
}

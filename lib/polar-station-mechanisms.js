...
// Catwalk/gantry deck between the drill shack and the borehole platform. It is
// still one continuously sampled surface built in symmetric endpoint-to-center
// bands (that is what the construction reveal rides), but the numbers now
// describe a walkway: 0.62 wide, a 60 mm plate, and a walkable camber rather
// than a ceremonial glass arch.
export const QPU_MANIFOLD_LAYOUT = Object.freeze({
  floorThickness: 0.06,
  foldLimit: 0.036,
  halfDepth: 0.31,
  halfSpan: 1.28,
...
export const NE_MONUMENT_CONTRACTS = Object.freeze({
  "s2-kernel-core": freezeMonumentContract({
    ...
    interaction: "approach spins up the cooling fans; dock storms the rack indicator banks behind the window strip",
    materialSignature: "insulated blue-grey panel hut on graphite steel stilt legs / machined pale metal cooling fan rotors / cobalt rack indicator lights behind a narrow window strip",
    silhouette: ["stilted server and data hut", "cooling louvre and fan bank", "rack indicator window strip"],
    topology: "compute runs inside an insulated box the camp keeps cold",
  }),
  "manifold-reactor": freezeMonumentContract({
    ...
    interaction: "proximity cracks the roller door; dock rolls it up on one contained burner",
    materialSignature: "abyss-blue panel machine hall on a graphite plinth / day tanks, switchgear and a plumed exhaust stack / one small contained living gold burner behind rolling door slats",
    silhouette: ["long low generator hall", "plumed exhaust stack and day tanks", "roller door over a contained burner"],
    topology: "power is made inside a hall and let out through one door",
  }),
  ...
  "qpu-ice-bridge": freezeMonumentContract({
    geometryPools: ["borehole-derrick-rig", "catwalk-handrail-bays", "cable-telemetry-run"],
    heroScale: 1.88,
    heroSpanSealWidths: 6.5,
    interaction: "route progress lays the catwalk from both ends; coherent dock runs the wireline sonde down the hole",
    materialSignature: "graphite steel drill derrick over a hazard-ringed borehole collar / desaturated panel drill shack with ember windows and a mint indicator panel / mint telemetry lights running the winch cable",
    silhouette: [
    ...
    topology: "the core is verified by going down the hole and coming back up the cable",
  }),
});
...
export function sampleQpuManifoldHeight(x, z) {
  const normalizedX = clamp(finite(x) / QPU_MANIFOLD_LAYOUT.halfSpan, -1, 1);
  const normalizedZ = clamp(finite(z) / QPU_MANIFOLD_LAYOUT.halfDepth, -1, 1);
  // Walkway camber, not a suspended arch: a shallow longitudinal crown, a
  // transverse fall so meltwater runs off the plate, and a low ripple that
  // keeps the deck from reading as a perfectly extruded ramp.
  const spanArch = 0.07 * (1 - normalizedX * normalizedX);
  const crossArch = 0.028 * (1 - normalizedZ * normalizedZ);
  const saddle = 0.012 * (normalizedZ * normalizedZ - normalizedX * normalizedX);
  const ripple =
    0.006 *
    Math.sin((normalizedX + 1) * Math.PI * 0.5) *
    Math.cos(normalizedZ * Math.PI);
  return spanArch + crossArch + saddle + ripple;
}

export function sampleQpuManifoldPoint(x, z, foldActivity = 0, time = 0) {
  const sourceX = finite(x);
  const sourceZ = finite(z);
  const y = sampleQpuManifoldHeight(sourceX, sourceZ);
  const activity = clamp(finite(foldActivity), 0, 1);
  const authoredTime = finite(time);
  const fold =
    Math.sin(y * 8 + sourceZ * 13 + authoredTime * 1.25) *
    Math.cos(sourceX * 9 - authoredTime * 0.85) *
    activity;
  return {
    x: sourceX + clamp(fold * 0.032, -QPU_MANIFOLD_LAYOUT.foldLimit, QPU_MANIFOLD_LAYOUT.foldLimit),
    y,
    z: sourceZ + clamp(fold * -0.024, -QPU_MANIFOLD_LAYOUT.foldLimit, QPU_MANIFOLD_LAYOUT.foldLimit),
  };
}

export function sampleQpuManifoldBoundary(
  sliceIndex,
  edge,
  transverse,
  foldActivity = 0,
  ...
  const boundedSlice = clamp(Math.trunc(finite(sliceIndex)), 0, QPU_MANIFOLD_LAYOUT.sliceCount - 1);
  const boundaryIndex = boundedSlice + (finite(edge) > 0 ? 1 : 0);
  const x =
    -QPU_MANIFOLD_LAYOUT.halfSpan +
    boundaryIndex * ((QPU_MANIFOLD_LAYOUT.halfSpan * 2) / QPU_MANIFOLD_LAYOUT.sliceCount);
  const z = clamp(finite(transverse), -1, 1) * QPU_MANIFOLD_LAYOUT.halfDepth;
  return sampleQpuManifoldPoint(x, z, foldActivity, time);
}

export function resolveQpuConstructionStep(progress, stepCount) {
  const boundedStepCount = Math.max(0, Math.trunc(finite(stepCount)));
  if (boundedStepCount === 0) return 0;
  return Math.min(
    boundedStepCount,
    Math.floor(clamp(finite(progress), 0, 1) * boundedStepCount + STEP_EPSILON),
  );
}

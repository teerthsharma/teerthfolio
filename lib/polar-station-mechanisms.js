...
export function sampleQpuManifoldHeight(x, z) {
  ...
  const spanArch = 0.34 * (1 - normalizedX * normalizedX);
  const crossArch = 0.16 * (1 - normalizedZ * normalizedZ);
  const saddle = 0.035 * (normalizedZ * normalizedZ - normalizedX * normalizedX);
  const ripple =
    0.018 *

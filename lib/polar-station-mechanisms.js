...
export const NE_MONUMENT_CONTRACTS = Object.freeze({
  ...
  "qpu-ice-bridge": freezeMonumentContract({
    geometryPools: ["paired-sanctums", "coherence-plate", "verification-conduit"],
    heroScale: 1.88,
    ...
    silhouette: ["stepped coherence span", "paired endpoints", "verification beam"],
    topology: "coherence crosses between sanctums",
  }),
});
...
function createQpuState() {
  return {
    ...
    evidenceReady: false,
    input: createInputBuffer(),
    naniteAssembly: new Float32Array(5),
    phase: "DECOHERENT",
    phaseAge: 0,
    plateLifts: new Float32Array(5),
    plateVelocities: new Float32Array(5),
...
function stepQpu(state, rawInput, reducedMotion) {
  ...
  if (reducedMotion) {
    state.naniteAssembly.fill(input.docked ? 1 : input.routeProgress);
    state.plateLifts.fill(1);
    state.plateVelocities.fill(0);
  ...
  for (let index = 0; index < state.plateLifts.length; index += 1) {
    const threshold = index / (state.plateLifts.length - 1);
    const target = input.routeProgress + STEP_EPSILON >= threshold ? 1 : 0;
    stepSpringAt(
      state.plateLifts,
      state.plateVelocities,
      index,
      target,
      9.2,
      1,
    );
    state.plateLifts[index] = clamp(state.plateLifts[index], 0, 1);
    state.plateVelocities[index] = clamp(state.plateVelocities[index], -5, 5);
    const naniteThreshold = index / (state.naniteAssembly.length - 1);
    const naniteTarget = input.docked
      ? 1
      : smoothstep01((input.routeProgress - naniteThreshold + 0.12) / 0.28);
    state.naniteAssembly[index] +=
      (naniteTarget - state.naniteAssembly[index]) *
      (1 - Math.exp(-FIXED_STEP_SECONDS * 8.6));
    state.naniteAssembly[index] = clamp(state.naniteAssembly[index], 0, 1);
  }

  let continuousSpan = true;
  for (let index = 0; index < state.plateLifts.length; index += 1) {
    if (state.plateLifts[index] < 0.94) {
  ...
  if (departing && state.phase !== "DECOHERENT") {
    ...
    state.evidenceReady = false;
    if (state.phaseAge > 0.7 && state.plateLifts[0] < 0.03) {

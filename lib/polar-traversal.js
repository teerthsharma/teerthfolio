...
/**
 * Turn rate falls off with speed, and a cornering seal scrubs some of it off.
 *
 * A constant rate corners as tightly at the 4 m/s cap as at walking pace, which
 * takes speed out of the decision entirely: there is nothing to trade for it and
 * nothing that punishes carrying it into a corner. The rate is scaled by
 * MANUAL_TURN_SPEED_FALLOFF across the speed range, and when the steer is
 * actually capped — the player asking for more turn than the body has — the
 * excess bleeds speed at MANUAL_CORNER_SCRUB per radian of unmet swing.
 */
export const MANUAL_TURN_SPEED_FALLOFF = Object.freeze({ atRest: 1.55, atCap: 0.72 });
export const MANUAL_CORNER_SCRUB = 0.55;
/**
 * How fast the velocity follows the body once the two disagree, as a fraction
 * of the forward correction. Below 1 the seal keeps travelling the way it was
 * pointed for a moment after the body has turned, which is the slide.
 */
export const MANUAL_LATERAL_GRIP = 0.34;
...
/**
 * Accelerate toward a target, with the correction across the target direction
 * damped by `grip`. At grip 1 this is exactly accelerateVelocity. The target is
 * the body heading, so the damped component is the difference between where the
 * seal points and where it is going — the slide.
 */
function accelerateVelocityWithGrip(state, targetX, targetZ, maximumDelta, grip) {
  const targetLength = Math.hypot(targetX, targetZ);
  if (targetLength <= STEP_EPSILON || grip >= 1) {
    accelerateVelocity(state, targetX, targetZ, maximumDelta);
    return;
  }
  ...
  const along = deltaX * forwardX + deltaZ * forwardZ;
  const across = (deltaX * -forwardZ + deltaZ * forwardX) * grip;
  const scaledX = forwardX * along + -forwardZ * across;
  const scaledZ = forwardZ * along + forwardX * across;
  const magnitude = Math.hypot(scaledX, scaledZ);
  if (magnitude <= STEP_EPSILON) return;
  const scale = Math.min(1, maximumDelta / magnitude);
  state.vx += scaledX * scale;
  state.vz += scaledZ * scale;
}
...
function stepTraversal(state, input, stations, colliders) {
  ...
  if (hasManualInput) {
    ...
    // Not while in contact. Collision resolution deflects the velocity along the
    // wall, and steering the body back toward the raw input from that deflected
    // heading fights it: the seal oscillates against the surface and a single
    // continuous press registers as repeated impacts. In contact, input maps
    // directly and the wall does the steering.
    if (currentSpeed > MANUAL_PIVOT_SPEED && !state.collisionId) {
      const currentAngle = state.heading;
      ...
      const speedFraction = clamp(currentSpeed / MANUAL_MAX_SPEED, 0, 1);
      const turnRate =
        MANUAL_TURN_RATE *
        (MANUAL_TURN_SPEED_FALLOFF.atRest +
          (MANUAL_TURN_SPEED_FALLOFF.atCap - MANUAL_TURN_SPEED_FALLOFF.atRest) *
            speedFraction);
      const maximumSwing = turnRate * FIXED_STEP_SECONDS;
      const steered =
        Math.abs(swing) > maximumSwing
          ? currentAngle + Math.sign(swing) * maximumSwing
          : targetAngle;
      state.heading = wrapAngle(steered);
      // Scrub: the turn the body could not make comes off the speed, so
      // carrying pace into a hard corner costs something.
      const scrubbed = Math.max(
        MANUAL_PIVOT_SPEED,
        MANUAL_MAX_SPEED -
          Math.max(0, Math.abs(swing) - maximumSwing) *
            MANUAL_CORNER_SCRUB *
            MANUAL_MAX_SPEED,
      );
      targetVelocityX = Math.cos(state.heading) * scrubbed;
      targetVelocityZ = Math.sin(state.heading) * scrubbed;
    } else {
      state.heading = wrapAngle(Math.atan2(targetVelocityZ, targetVelocityX));
    }
  } else if (state.route) {
  ...
  accelerateVelocityWithGrip(
    state,
    targetVelocityX,
    targetVelocityZ,
    acceleration * FIXED_STEP_SECONDS,
    // Grip is piloted-only, and off in contact where the wall owns the
    // direction. Guided routing keeps its authored cornering.
    hasManualInput && !state.collisionId ? MANUAL_LATERAL_GRIP : 1,
  );
...
export function createTraversalState({ x = 0, z = 0, vx = 0, vz = 0 } = {}) {
  ...
  return {
    ...
    dockedId: null,
    // The body's facing, held separately from the velocity so the two can
    // disagree. Without it there is nothing for a drifting velocity to lag
    // behind: steering that rotates the target velocity gives the character
    // exactly one direction, and damping its one correction only slows the turn.
    heading: Math.atan2(finite(vz), finite(vx)) || 0,
    impactSequence: 0,
...
export function getTraversalRenderPose(state) {
  ...
  return {
    // Body facing, published alongside the velocity because through a corner
    // they differ and the renderer needs the body, not the slide.

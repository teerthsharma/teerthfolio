...
function stepTraversal(state, input, stations, colliders) {
  ...
  if (hasManualInput) {
    ...
    // wall, and steering the target back toward the raw input from that deflected
    ...
    if (currentSpeed > MANUAL_PIVOT_SPEED && !state.collisionId) {
      ...
      if (Math.abs(swing) > maximumSwing) {
        ...
        const scrubbed = Math.max(
          MANUAL_PIVOT_SPEED,
          MANUAL_MAX_SPEED -
            (Math.abs(swing) - maximumSwing) * MANUAL_CORNER_SCRUB * MANUAL_MAX_SPEED,
        );
        targetVelocityX = Math.cos(steered) * scrubbed;
        targetVelocityZ = Math.sin(steered) * scrubbed;
      }
    }
  } else if (state.route) {

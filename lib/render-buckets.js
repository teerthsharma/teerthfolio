 ...
 *
 * The reason is that the load fix has two independent halves and this is the
 * lesser one. Specialising the biome programs so the sky role compiles its own
 * branch removed the link cost at its source, and that applies to every path
 * regardless of how the scene is admitted. Bucketing spreads what remains across
 * frames the viewer is already watching. A visitor who asks for reduced motion
 * gets the same time to first frame as everyone else, which is the outcome that
 * matters and was previously only assumed.
 *

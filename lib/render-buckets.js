 ...
 *
 *   cold, still the entry splash      1,707ms from navigation,   331ms after click
 *   cold, fully composed              1,826ms from navigation,   393ms after click
 *   warm, renderer reports gl                                    716ms after click
 *
 * Composed means the authored world: full dome masonry, the tunnel, terrain
 * drift, the mountain range, the aurora, the mascot and the HUD. Confirmed by
 * eye at 1,919ms and by the splash capture at 1,612ms, so the transition is
 * bracketed to a 119ms window rather than inferred.
 *
 ...
 *
 * That exemption used to be justified by assuming those paths had the least to
 * draw. Measured instead, on cold profiles with an empty GPU program cache, they
 * cost the same as the staged path:
 *
 *   normal           world composed 1,929ms from navigation, 527ms after click
 *   reduced motion   world composed 1,931ms from navigation, 560ms after click
 *
 * The reason is not that specialising the biome programs removed the link cost —
 * an earlier note here claimed that and it is wrong. The BIOME_ROLE_SKY define
 * cut the fragment source but was measured at the time to leave link time
 * unchanged, and it still is: the two programs cost 2,570ms and 1,608ms to link
 * on a cold visit. What makes the world appear quickly is that neither is
 * required for it. The hero bucket draws the igloo, the character and a cheap
 * stand-in ground, and the biome world arrives a bucket later.
 *
 * Which means the stall was moved rather than removed, and it lands on a visitor
 * who is already looking at the world. See the note on sky admission in
 * IglooScene: those two links used to arrive in the same frame.
 *
 * Ablated directly — shouldStageRenderBuckets forced to false on the normal path,
 * cold profiles, same bracket method — bucketing costs nothing and buys nothing
 * that can be measured here:
 *
 *   staged      world composed 393ms after the click
 *   unstaged    world composed 429ms and 430ms after the click, splash at 301ms
 *
 * That is inside the run-to-run spread. So on this machine the staging is
 * currently carrying its complexity for no measured return, and a reader
 * deciding whether to keep it should know that. Two reasons it is still here.
 * It was measured to help before the programs were specialised, against an 11.4s
 * link stall, so it is insurance against that class of regression returning. And
 * the machine it fails to help on links 76 programs in about 2.3s; a slower
 * shader compiler, a colder cache, or more programs move the balance back.
 *
 * If it is removed, remove it on a measurement rather than on this comment.
 */

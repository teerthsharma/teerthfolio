 ...
 */
export const MECHANISM_VERTICAL_ENVELOPES = Object.freeze({
  "observatory-plaque": Object.freeze({
    coreFootprintScale: Object.freeze({ x: 0.55, z: 0.465 }),
    // Measured off the lattice rather than guessed, because both errors bite.
    // The rendered dome reaches 2.10 x 1.58 = 3.32 in x and 1.86 x 1.58 = 2.94
    // in z, plus the course blocks that stand proud of the shell (0.21 x 1.58)
    // and their lay jitter (0.075 x 1.58): 3.77 by 3.39 against a 4.4 x 3.2
    // collider. The old 1.12 x 0.94 was measured against a 2.42 x-radius
    // lattice, so after the dome narrowed and rose it was half again too wide
    // and a tenth too shallow — the chase lens read itself as buried while
    // standing in open snow, and separately skimmed the dome's north face with
    // the whole building across the shot while the occlusion test reported
    // clear.
    footprintScale: Object.freeze({ x: 0.88, z: 1.06 }),
...
/**
 * The seal's measured presence, published so the ground shadow solves against
 * the same body the camera frames rather than a second guess at its size.
 */
export const SEAL_ENVELOPE = Object.freeze({
  axialRadius: SEAL_AXIAL_RADIUS,
  lateralRadius: SEAL_LATERAL_RADIUS,
  verticalRadius: SEAL_VERTICAL_RADIUS,
  worldY: SEAL_WORLD_Y,
});
...
export function solveCameraOcclusionFactor({
  ...
  // Walk OUTWARD from the seal and stop at the first obstructed sample. The
  // direction matters and this used to walk inward, returning the first
  // unobstructed sample it met coming down from full distance — which answers
  // "is the lens itself inside a building", not "is the building between the
  // lens and the subject". A camera parked beyond the far side of the dome sits
  // in open snow, so the inward walk cleared it at the very first sample and
  // reported no occlusion while the whole building filled the shot.
  for (let step = 1; step <= CAMERA_OCCLUSION_SAMPLES; step += 1) {
    ...
    for (let index = 0; index < CAMERA_OCCLUDERS.length; index += 1) {
      if (insideOccluder(CAMERA_OCCLUDERS[index], x, y, z)) {
        // Blocked at the very first sample means the seal is standing inside a
        // facility's envelope — docked against its flank, or piloted into the
        // igloo through the airlock. There is no shorter offset that clears the
        // building, so shortening only buries the lens in the wall it was
        // trying to see past. Hold the authored distance instead.
        if (step === 1) return 1;
        return Math.max(
          CAMERA_OCCLUSION_MIN_FACTOR,
          (step - 1) / CAMERA_OCCLUSION_SAMPLES,
        );
      }
    }
  }
...
export function solvePolarCameraComposition({
  ...
  // The ceiling is headroom for the solver, not a target: candidates are scored
  // with distance added to the penalty, so every station still settles at the
  // closest framing that satisfies the inset and span contract. It rose when the
  // observatory dome grew to real igloo proportion (4.8 world units at the
  // crown) and 15.5 could no longer clear the 48px hero inset above it.
  const maximumDistance = portrait ? 22 : 18.5;

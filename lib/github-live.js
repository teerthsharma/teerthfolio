...
// Every one of these calls is awaited server-side before the page renders, and
// none of them had a deadline: a hanging GitHub was still pending at 5,006ms in
// test, which means a visitor waits out the serverless function timeout with
// nothing to show. Each fetch already has a fallback; this gives it a trigger.
...
export async function fetchRepositoryMetrics({ fullName, fallbackSnapshot } = {}) {
  ...
  try {
    ...
    const treeResponse = await fetch(
      ...
      { headers, next: { revalidate: 3600 }, signal: AbortSignal.timeout(GITHUB_FETCH_TIMEOUT_MS) },
    );

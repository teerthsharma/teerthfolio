...
export const SEAL_FUR_TIER_COUNT = Object.freeze({ low: 28, medium: 40, high: 40 });

/** Deterministic hash in [0,1) seeded from an integer cursor — no Math.random. */
function furHash(seed) {
  const value = Math.sin(seed * 127.1 + 311.7) * 43758.5453123;
  return value - Math.floor(value);
}

/**
 * Deterministic hairstyle anchor sampling over the seal crown. Anchors are
 * picked by hash-seeded vertex indices restricted to the crown/back-of-head
 * band above and behind the face patch, with a small tangential jitter. The
 * body stays sleek: no anchor exists outside the crown zone. Same geometry +
 * tier always yields byte-identical placements.
 */
export function createSealFurPlacements(geometry, quality = "medium") {
  const count = SEAL_FUR_TIER_COUNT[quality] || SEAL_FUR_TIER_COUNT.medium;
  const position = geometry.getAttribute("position");
  const normal = geometry.getAttribute("normal");
  const canonical = geometry.getAttribute("canonical");
  if (!position || !normal || !canonical) {
    throw new Error("seal hair sampling requires position, normal, and canonical attributes");
  }
  const placements = [];
  let cursor = 0;
  while (placements.length < count) {
    cursor += 1;
    const vertex = Math.floor(furHash(cursor) * position.count) % position.count;
    const crownX = canonical.getX(vertex);
    const crownY = canonical.getY(vertex);
    // Crown zone only: behind the face patch, ahead of the shoulders, upper
    // hemisphere. Everything else stays a sleek bald seal surface.
    if (crownX > 0.8 || crownX < 0.34 || crownY < 0.05) continue;
    const nx = normal.getX(vertex);
    const ny = normal.getY(vertex);
    const nz = normal.getZ(vertex);
    // Tangent frame for deterministic root jitter so shared vertices diverge.
    const refY = Math.abs(ny) < 0.94 ? 1 : 0;
    const refZ = refY === 1 ? 0 : 1;
    ...
    const jitterT = (furHash(cursor * 2.17 + 9.13) - 0.5) * 0.05;
    const jitterB = (furHash(cursor * 3.31 + 4.71) - 0.5) * 0.05;
    placements.push({
      position: [
        position.getX(vertex) + tx * jitterT + bx * jitterB,
        position.getY(vertex) + ty * jitterT + by * jitterB,
        position.getZ(vertex) + tz * jitterT + bz * jitterB,
      ],
      normal: [nx, ny, nz],
      canonicalX: canonical.getX(vertex),
      canonicalY: canonical.getY(vertex),
      lengthJitter: furHash(cursor * 5.13 + 1.37),
      lean: furHash(cursor * 7.77 + 2.53),
      shade: furHash(cursor * 11.29 + 6.19),
    });
  }
  return placements;
}

...
${SHADOW_CASTERS.map(
  ...
  if (dot(travelerDelta, travelerDelta) < ${glslNumber(
    ...
    shadow = max(shadow, polarShadowFromEllipsoid(
      ...
      vec3(
        uTravelerXZ.x,
        ${glslNumber(SEAL_ENVELOPE.worldY)} + polarGroundHeight(uTravelerXZ),
        uTravelerXZ.y
      ),
      vec3(${glslNumber(
        Number(Math.sqrt(SEAL_ENVELOPE.axialRadius * SEAL_ENVELOPE.lateralRadius).toFixed(4)),
      )}, ${glslNumber(SEAL_ENVELOPE.verticalRadius)}, ${glslNumber(
        Number(Math.sqrt(SEAL_ENVELOPE.axialRadius * SEAL_ENVELOPE.lateralRadius).toFixed(4)),
      )})
    ));
  }

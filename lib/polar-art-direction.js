...
export const POST_PROCESS_BUDGET = Object.freeze({
  ...
  //
  //   medium   +3.9% gradient   deltas +0.16 +0.16 +0.16 +0.16   sd 0.001
  //            ringing 0.00% of edge pixels, no clipping
  //
  // Four identical deltas, and the earlier figures on a live scene — +13%, then
  // +3.2%, then about 1% ringing — were all measuring the aurora as much as the
  // filter. Low does not resolve under the freeze: something there still moves
  // by the fourth capture, so its arm reads sd 0.298 against a 0.239 effect and
  // the probe correctly refuses it.
  //

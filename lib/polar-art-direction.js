...
export const POST_PROCESS_BUDGET = Object.freeze({
  ...
  //
  //   medium   +3.9%   median delta +0.161   mad 0.001   ringing 0.00%
  //   low      +5.2%   median delta +0.224   mad 0.000   ringing 0.00%
  //
  // Neither tier clips highlights; low clips 0.058% of shadows. The earlier
  // figures on a live scene — +13%, then +3.2%, then about 1% ringing — were
  // measuring the aurora as much as the filter.
  //
  // Both are medians over four pairs rather than means, because about one
  // capture in four lands on a transient the clock freeze does not cover and a
  // single such frame moves the mean further than the effect: at medium the mean
  // reads +0.045 against a median of +0.161. The concordant pairs agree to a
  // median absolute deviation of 0.001, so this is a precise measurement of a
  // small effect rather than a rough one.
  //
  ...
  low: Object.freeze({
    ...
    pixel: 1,
    quantize: 0.04,
    // The paper grade and its shadow toe stay as authored. They are what the
    // 0.82 render scale is compensated by, and they are not what made the world
    // read as a diagram — the ink outline and the quantize step were.
    gradeBase: 0.04,
    gradeCurve: 0.9,

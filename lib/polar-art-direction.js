...
export const POST_PROCESS_BUDGET = Object.freeze({
  ...
  //
  // Ringing could not be measured to the same standard and the earlier figures
  // here overstated their own certainty. Per-pixel overshoot compares two
  // browser sessions that cannot be phase-locked, so a moving aurora reads as
  // overshoot; across four pairs at low it ran 0.94, 0.48, 0.77, 2.27 percent,
  // which is the pairing decaying rather than the filter worsening. Treat about
  // 1% of edge pixels as an upper bound, not a measurement. What can be said is
  // that nothing clips and the gain is small.
  //

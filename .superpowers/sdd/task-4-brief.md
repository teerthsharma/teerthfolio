...
Share a design grammar—contained core, readable frame, layered computation—but invert topology: S2 closes state inward; Aether circulates phase outward. Give both real support/contact and remove generic planet/ring silhouettes.

- [ ] **Step 3: Rebuild Field and QPU**

Give Field a chamber boundary with compressed coil field and QPU a traversable architectural span, not floating rings/plates. Use bounded Fresnel/noise/wrapped diffuse math from the local hologram research without copying WebGPU-only code or assets.
